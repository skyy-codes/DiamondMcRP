...
def is_grayscale(pix):
    return pix[0] == pix[1] == pix[2]

def tint_cyan(pix, factor=0.5):
    r, g, b, a = pix
    if a == 0: return pix
    nr = int(r * (1 - factor))
...
def process_diamond(input_path, output_path, cyan_ratio=0.4):
...
def process_reinforced(input_path, output_path, factor=0.4):
    if not os.path.exists(input_path): return
    img = Image.open(input_path).convert("RGBA")
    pixels = img.load()
    width, height = img.size
...
# Diamond Armor Model (Subtle Cyan)
